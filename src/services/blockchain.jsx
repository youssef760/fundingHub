import abi from '../abis/src/contracts/FundingHub.sol/FundingHub.json'; // Replace with the correct path and filename
...
const startProject = async ({
  ...
  try {
    ...
    tx = await contract.startProject(title, description, imageURL, cost, expiresAt);
    await tx.wait();
    await loadProjects();
  } catch (error) {
    reportError(error);
...
const modifyProject = async ({
  ...
  try {
    ...
    const contract = await getEthereumContract();
    tx = await contract.modifyProject(id, title, description, imageURL, expiresAt);
    await tx.wait();
...
const cancelProject = async (id) => {
  try {
    if (!ethereum) return alert('Please install Metamask');
    const contract = await getEthereumContract();
    await contract.cancelProject(id);
  } catch (error) {
    reportError(error);
  }
};
...
const loadProject = async (id) => {
  try {
    if (!ethereum) return alert('Please install Metamask');
    const contract = await getEthereumContract();
    const project = await contract.getProjectDetails(id);
...
const contributeToProject = async (id, amount) => {
  try {
    ...
    tx = await contract.contributeToProject(id, {
...
const getBackers = async (id) => {
  try {
    if (!ethereum) return alert('Please install Metamask');
    const contract = await getEthereumContract();
    let backers = await contract.getProjectBackers(id);
